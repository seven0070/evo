...
> The categories above are enforced as a file set. As of P4 the manifest
> (`evo_agent/sovereign/protected.py`, mirrored in `sovereign.manifest.json`) enumerates **20** paths: the
> `sovereign/` package (`architecture`, `eligibility`, `invariants`, `mediation`, `protected`), the ten
> governance-and-runtime modules it already named - and, newly, `evo_agent/active_version.py` and
> `evo_agent/materialization.py`. Those two are the application side of the same promise the rollback
> authority makes: a loop that can widen what an overlay may say, or choose which materializer validates it,
> can reach protected source through configuration rather than through an edit. P3 left them ungated because
> no overlay existed to widen; P4 gave them a consumer, so the gate was widened in the same change and the
> digest re-published, with the count asserted by `I-sovereign-coverage`.
>
> **P5 changed the file set by zero and the checks by one.** The count above is still 20. What P5 added is
> `I-ownership-boundary`, backed by `evo_agent/upstream.py`: the table that states who owns each of the 23
> capabilities, which upstream components were reviewed at which ref, and which paths would prove something
> was vendored. Two modules that P5 could plausibly have protected - `skills.py`, the skill loader, and
> `tools.py`, the canonical-name view - were deliberately **not** added. The alternative to protecting them
> is not "leave them unguarded": the invariant requires every capability claimed as `sovereign` to name an
> authority that is either in the byte set or imported by something in it, and the row's `reason` names the
> clamp that makes that enough (`materialization.py` refuses an ordering function; `security.py` clamps
> every ceiling). Widening the byte set stays a governance act, and a capability phase does not get to
> perform it as a side effect of convenience - which is the mistake the count above exists to make visible.
