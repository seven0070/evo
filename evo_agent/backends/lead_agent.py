...
class LeadAgentBackend:
    ...
    def _service_tool_request(self, process: "subprocess.Popen[str]", message: dict[str, Any], turn: _Turn, context: TurnContext) -> None:
        ...
        result = self.mediator.execute(request, tool_name=tool_name or "shell", arguments=arguments) if self.mediator else None
        if result is None:
            ...
            turn.receipts.append(
                Receipt.record(
                    ...
                    canonical_name=f"shell.{tool_name or 'exec'}",
                    kind="execute",
