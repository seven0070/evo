...
TARGET_KINDS: tuple[TargetKind, ...] = (
    ...
    TargetKind(
        ...
        payload="heuristic weights and stage selection supplied as data",
        loadable=True,
        benchmark_suites=("core-local",),
        phase="P4",
        notes=(
            "P4 closed this: config/heuristics.json is read by TurnPipeline.from_overlay, bounded to the "
            "eight reviewed knobs, and ordering *functions* remain unmaterializable by construction"
        ),
        sandbox_accepted=True,
    ),
    TargetKind(
        name="prompt/configuration parameters",
        payload="provider settings from the reviewed allow-list; prompt text is not an evolution payload",
        loadable=False,
        benchmark_suites=("core-local", "research"),
        phase="never",
        notes=(
            "P5 resolved this as a decision rather than a schedule: prompt text authored by the agent to "
            "change its own guardrails is out of scope (03 §E), so no phase delivers a loader and "
            "ProviderConfigMaterializer keeps refusing the document. The R7 isolation-downgrade rule is "
            "enforced independently, and is the reason this row still exists"
        ),
        sandbox_accepted=True,
    ),
    TargetKind(
        ...
        phase="P5",
        notes=(
            "P5 built the catalog, and the catalog delegates validation to SkillMaterializer instead of "
            "copying it - name shape, frontmatter, size, line count, executable markers. An installed bundle "
            "is staged for review and mounted read-only while it is enabled; it authorises nothing, and a "
            "required secret stays a name the caller resolves rather than a value in a prompt"
        ),
        sandbox_accepted=True,
    ),
    TargetKind(
    ...
    TargetKind(
        name="memory_policy",
        payload="retrieval ranking weights, as a map over the closed component list",
        loadable=True,
        benchmark_suites=("memory-recall", "long-horizon"),
        phase="P5",
        notes=(
            "P5 delivered the consumer: RetrievalEngine ranks by the weights, the runtime applies them per "
            "cycle, and withdrawal restores the launch values. Retention and staleness stay operator-only "
            "(they expire rows, which a rollback cannot un-do), and the compaction budget belongs to "
            "config/heuristics.json rather than being a second knob for the same ceiling"
        ),
        # The sandbox accepts the target name today, so a candidate can be staged, benchmarked, approved
        # and promoted for this kind; marked here so I-eligibility-coherence demands the two agree.
        sandbox_accepted=True,
    ),
)
...
def consistency_with_sandbox() -> list[str]:
    ...
    for kind in TARGET_KINDS:
        ...
        try:
            ...
            continue
        if kind.loadable:
            # A loader claim is checked against the document table, not against a phase number. The
            # original rule was ``loadable and phase > P3``, which encoded "P3 is the last phase that may
            # add a loader": true when it was written, false from P4 onward, and a rule that fires on
            # honest bookkeeping is a rule that gets routed around. The defect that is real is a kind
            # claiming the runtime loads something while no document says who loads it - unaudited
            # capability - and that is checkable from one table. Strictly stronger than the cutoff in
            # every direction except the one where the cutoff was simply wrong.
            from ..active_version import TARGET_TO_KIND
            from ..materialization import loadable_kinds

            payload_kind = TARGET_TO_KIND.get(kind.name, kind.name)
            if payload_kind not in loadable_kinds():
                defects.append(
                    f"'{kind.name}' claims to be loadable but no document in active_version.DOCUMENTS "
                    f"names a loader for payload kind '{payload_kind}'"
                )
