...
_EXPLICIT_PROTECTED_PATHS: tuple[str, ...] = (
    ...
    "evolver.py",
    "metamorphosis.py",
    # What promotion *is*, from both sides. These two are in the byte set because an agent that can
    # rewrite the reader of the active version, or the writer of an overlay, can decide what "the
    # approved candidate" means without touching a single protected authority: ``active_version.py``
    # resolves and applies the overlay and owns the activation record that a rollback re-verifies
    # against, and ``materialization.py`` is the only path from an approved payload to bytes on disk.
    # Both were built in P3 and deliberately left ungated then, because widening the protected set is a
    # governance change and not a refactor (08 §P3, deliberate item); P4 makes it explicitly.
    "active_version.py",
    "materialization.py",
