...
def test_the_ledger_holds_exactly_the_one_open_defect():
    """P1 closed three links and P2 closed isolation; only the verifier's P4 defect is left.

    If this fails, a phase finished early - update the ledger in
    ``tests/test_audit_defects_characterisation.py`` in the same change, so the schedule and
    ...
    assert open_defect_count(text) == 1, open_defects
    assert open_defects == ["test_verifier_refuses_an_expectation_it_cannot_check"], open_defects
    assert "P4" in text, "every open defect must name the phase that must close it"
    # P2's promise is now asserted positively, so the ledger must not still mention it as open.
