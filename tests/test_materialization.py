...
def test_no_payload_for_an_unloaded_document_reaches_the_disk(workspace: Path):
    ...
    Four documents are declared with the fields they will one day carry and are refused on write, with
    ...
    assert set(unloaded) >= {"config/strategy.json", "config/memory.json", "config/prompts.json", "config/heuristics.json"}
...
def test_an_isolation_weakening_payload_is_refused_even_though_its_document_is_gated(workspace: Path):
    ...
    ``provider_config`` is refused by the loader gate today. Its materializer additionally refuses any
    payload naming a weaker isolation provider (R7). Pinning that the *gate* fires first keeps the P5
    plan honest: when the document becomes loadable, the R7 check is already tested and already in the
    path, rather than something to remember later.
    """
    result = materialize(
    ...
    assert not result.ok
    assert "nothing loads it before" in " ".join(result.errors)
...
def test_a_skill_is_validated_in_full_and_still_refused_on_write(workspace: Path):
    """The skill checks exist so P5 inherits them; the refusal exists so P3 has no code path to source.

    Both halves matter. Delete the refusal and P3 grows a self-modification path; delete the validation
    and P5 will write whatever its own ad-hoc checks accept, with the corpus of tests here pointing at a
    shape nobody enforces.
    """
    ...
    assert materializer is not None
    problems = materializer.validate(payload)
    assert problems == [f"{SKILL_SPEC.relpath}: nothing loads it before {SKILL_SPEC.phase} (kind 'skill' is declared for {SKILL_SPEC.phase}; refusing rather than writing dead config)"] or all(
        "nothing loads it before" in item for item in problems
    )
    with pytest.raises(MaterializationError):
        materializer.write_candidate(payload, workspace / "candidate")
    assert not (workspace / "candidate" / OVERLAY_DIRNAME / "capabilities").exists()
    result = materialize("skill", payload, workspace / "candidate")
    assert not result.ok
