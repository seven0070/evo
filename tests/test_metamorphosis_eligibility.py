...
def test_a_loadable_claim_names_a_document_the_runtime_reads():
    ...
    from evo_agent.materialization import SKILL_SPEC, loadable_kinds, materializer_for

    # ``loadable_kinds`` rather than ``DOCUMENTS`` alone, because P5 added a payload that is not a JSON
    # document (a skill bundle) and gave it a real loader. Deriving the set from the table the resolver
    # uses is the point; deriving it from one file's dictionary is how the check would start lying the
    # moment a second non-document kind appeared.
    loaded_kinds = set(loadable_kinds())
    ...
    for kind in TARGET_KINDS:
        ...
        owned = [DOCUMENTS[path] for path in materializer.owned_documents if path in DOCUMENTS]
        if not owned and mapped == SKILL_SPEC.kind:
            # A bundle is a file tree, not a JSON document, so its spec is not a ``DOCUMENTS`` row and it
            # cannot be listed in ``owned_documents`` either (that list is cross-checked against the table).
            # Naming the spec here keeps the rule "a loadable claim has an owner that accepts the payload"
            # true for the one kind the table does not describe as a document.
            owned = [SKILL_SPEC]
        assert any(spec.loadable for spec in owned), (
